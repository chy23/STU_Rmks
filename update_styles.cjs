...
const stylesString = "const STYLES = [\n" + newStyles.map(s => `  { name: "${s.name}", desc: "${s.desc}" }`).join(",\n") + "\n];";

// Using regex to replace the old STYLES array
const updatedContent = content.replace(/const STYLES = \[\s*\{[\s\S]*?\];/m, stylesString);
